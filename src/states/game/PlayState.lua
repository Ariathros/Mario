...
function PlayState:init()
    ...
    self.player = Player({
        -- x is set to a column where there is definite ground and multiply it by 16 to get the coordinate
        y = 0, x = self:getPlayerSpawnColumn() * 16,
        width = 16, height = 20,
        texture = 'green-alien',
...
function PlayState:enter(params)
    ...
    -- increase width of tilemap
    self.level = LevelMaker.generate(params.mapWidth + 20, 10)
    self.tileMap = self.level.tileMap
...
function PlayState:render()
    ...
    if self.player.hasKey then
       love.graphics.draw(gTextures['keys-locks'], gFrames['keys-locks'][1], VIRTUAL_WIDTH - 24, 4)
...
-- spawn to the closest safe column (has definite ground) from the left portion of the map
